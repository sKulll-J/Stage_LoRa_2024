...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  HAL_UART_Transmit(&huart2,(uint8_t*)"Set\r\n",5, 10);

	  if(verifcation_rep()==1)
	  {
		  uint16_t val1 = DHT11_Read();
		  uint16_t val2 = DHT11_Read();
		  char txt[30];
	  ...
	  char msg[20];
	  sprintf(msg, "%ld\r\n",cnt);
	  /*__HAL_TIM_SET_COUNTER(DHT_GetDt().tim,0);  // set the counter value a 0
	  	while (__HAL_TIM_GET_COUNTER(DHT_GetDt().tim) < 10000);
	  	*/
